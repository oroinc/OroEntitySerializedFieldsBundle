...
class EntityConfigListener
{
    ...
    public function postFlush(PostFlushConfigEvent $event)
    {
        ...
        foreach ($event->getModels() as $model) {
            ...
            if ($fieldConfig->is('is_serialized')) {
                $toUpdate[$className] = $this->getEntityConfig($configManager, $className);
            }
        }
    ...
    {
        return $configManager->getEntityConfig('extend', $className);
    }

    /**
     * Method preserved for backwards compatibility
     *
     * @param ConfigManager $configManager
